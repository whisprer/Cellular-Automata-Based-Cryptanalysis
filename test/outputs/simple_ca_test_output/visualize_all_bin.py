...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python visualize_bin.py <pattern_or_file>")
        print("Examples:")
        print("  python visualize_bin.py file1.bin file2.bin")
        print("  python visualize_bin.py \"*.bin\"")
        sys.exit(1)
    
    # Handle wildcards ourselves
    files = []
    ...
    if not files:
        print(f"No files found matching the patterns: {' '.join(sys.argv[1:])}")
        sys.exit(1)
