...
TestResult UniversalTest::execute(const BitSequence& data) {
    ...
    int Q = 10 * (1 << L);
    int K = static_cast<int>(data.size() / L) - Q;  // Number of blocks to test
    
    // Check if we have enough data for this test
    if (L < 6 || L > 16 || data.size() < static_cast<size_t>(Q * L)) {
        result.p_value = 0.0;
    ...
    for (int i = 0; i < Q; i++) {
        int pattern = 0;
        for (int j = 0; j < L; j++) {
            pattern = (pattern << 1) | static_cast<int>(data[i * L + j]);
        }
    ...
    for (int i = Q; i < Q + K; i++) {
        ...
        sum += std::log2(static_cast<double>(i + 1 - position));
    ...
    // Store results
    result.statistics["L"] = static_cast<double>(L);
    result.statistics["Q"] = static_cast<double>(Q);
    result.statistics["K"] = static_cast<double>(K);
    ...
    result.statistics["discarded_bits"] = static_cast<double>(data.size() - (Q + K) * L);
